...
<%@ page
        import="org.wso2.carbon.identity.application.mgt.defaultauth.stub.IdentityDefaultSeqManagementServiceDefaultAuthSeqMgtClientException" %>
...
<%
    ...
    if (StringUtils.isNotEmpty(seqContent)) {
        try {
            ...
            String message = resourceBundle.getString("info.add.artifact");
...
<%
    ...
    String[] errorMessages = e.getFaultMessage().getDefaultAuthSeqMgtClientException().getMessages();

    if (ArrayUtils.isNotEmpty(errorMessages)) {
        session.setAttribute("createError", errorMessages);
%>
<script>
    location.href = 'add-default-authSeq.jsp?createError=true';
</script>
<%
} else {
    String message = resourceBundle.getString("error.add.artifact");
