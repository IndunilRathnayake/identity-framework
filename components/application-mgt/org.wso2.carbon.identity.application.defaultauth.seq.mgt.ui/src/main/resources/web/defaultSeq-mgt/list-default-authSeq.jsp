...
<%@ page import="org.wso2.carbon.identity.application.defaultauth.seq.mgt.ui.DefaultAuthSeqMgtUIConstants" %>
...
<fmt:bundle
        basename="org.wso2.carbon.identity.application.defaultauth.seq.mgt.ui.i18n.Resources">
        ...
        <div id="workArea">
            ...
            <%
                ...
                try {
                    ...
                    sequence = serviceClient.getDefaultAuthenticationSeq(
                            DefaultAuthSeqMgtUIConstants.DEFAULT_AUTH_SEQ_RESOURCE_NAME);
                } catch (Exception e) {
            ...
            <table style="width: 100%" class="styledLeft">
                <div style="height:30px;">
                    <%
                        if (sequence == null || StringUtils.isBlank(sequence.getContent())) {
                    %>
                        ...
                        <table class="styledLeft" width="100%" id="defaultAuthnSequences">
                            ...
                            <%
                                boolean canView = CarbonUIUtil.isUserAuthorized(request,
                                        "/permission/admin/manage/identity/resourcemgt/view");
                                boolean canEdit = CarbonUIUtil.isUserAuthorized(request,
                                        "/permission/admin/manage/identity/resourcemgt/update");
                                boolean canDelete = CarbonUIUtil.isUserAuthorized(request,
                                        "/permission/admin/manage/identity/resourcemgt/delete");
                                if (sequence != null && StringUtils.isNotBlank(sequence.getContent())) {
                            %>
                            <tbody>
                            <tr>
                                ...
                                <td style="width: 100px; white-space: nowrap;">
                                    ...
                                    <a title="Edit Default Authentication Sequence"
                                       onclick="javascript:location.href=
                                               'edit-default-authSeq.jsp?seqName=<%=Encode.forUriComponent(sequence.getName())%>'"
                                       class="icon-link"
                                    ...
                                    <a title="Export Default Authentication Sequence"
                                       onclick="exportDefaultAuthSeq('<%=Encode.forJavaScriptAttribute(sequence.getName())%>');"
                                       class="icon-link"
                                    ...
                                    <a title="Remove Default Authentication Sequence"
                                       onclick="removeDefaultAuthSeq('<%=Encode.forJavaScriptAttribute(sequence.getName())%>');"
                                       class="icon-link"
