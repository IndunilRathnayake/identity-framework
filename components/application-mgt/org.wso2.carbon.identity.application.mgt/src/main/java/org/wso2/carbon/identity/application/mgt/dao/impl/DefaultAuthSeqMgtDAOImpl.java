...
public class DefaultAuthSeqMgtDAOImpl implements DefaultAuthSeqMgtDAO {
    ...
    public void createDefaultAuthenticationSeq(DefaultAuthenticationSequence sequence, String tenantDomain)
        ...
        try {
            ...
            throw new DefaultAuthSeqMgtException("Error while creating default authentication sequence " +
    ...
    public DefaultAuthenticationSequence getDefaultAuthenticationSeq(String tenantDomain)
        ...
        try {
            ...
            throw new DefaultAuthSeqMgtException("Error while retrieving default authentication sequence " +
                    "in tenant: " + tenantDomain, e);
        }
    }

    @Override
    public boolean isDefaultAuthSeqExists(String tenantDomain) throws DefaultAuthSeqMgtException {

        try {
            int sequenceID = doGetSequenceID(tenantDomain, null);
            if (sequenceID != 0) {
                return true;
            }
        } catch (DataAccessException e) {
            throw new DefaultAuthSeqMgtException("Could not check existence of default authentication " +
                    "sequence in tenant: " + tenantDomain, e);
        }
        return false;
    }
    ...
    public void deleteDefaultAuthenticationSeq(String tenantDomain) throws DefaultAuthSeqMgtException {
        ...
        try {
            ...
            throw new DefaultAuthSeqMgtException("Error while deleting default authentication sequence " +
                    "in tenant: " + tenantDomain, e);
        }
    }
    ...
    private int doCreateDefaultAuthSeq(DefaultAuthenticationSequence sequence, String tenantDomain,
                                 JdbcTemplate jdbcTemplate) throws DataAccessException {

        return jdbcTemplate.executeInsert(ADD_DEFAULT_SEQ, (preparedStatement -> {
    ...
    private DefaultAuthenticationSequence doGetDefaultAuthSeq(String tenantDomain, JdbcTemplate jdbcTemplate)
        ...
        return jdbcTemplate.fetchSingleRecord(GET_DEFAULT_SEQ,
                (resultSet, rowNumber) -> {
                    ...
                    sequence.setDescription(resultSet.getString(1));
                    try {
                        ...
                        if (objectRead != null && objectRead instanceof LocalAndOutboundAuthenticationConfig) {
    ...
    private void doUpdateDefaultAuthSeq(DefaultAuthenticationSequence sequence, String tenantDomain,
        ...
        jdbcTemplate.executeUpdate(UPDATE_DEFAULT_SEQ,
                preparedStatement -> {
                    preparedStatement.setString(1, sequence.getDescription());
                    try {
